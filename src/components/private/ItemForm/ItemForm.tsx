...
import type { Employee } from '../data/item';

export interface ItemFormState {
    values: Partial<Omit<Employee, 'id'>>;
    errors: Partial<Record<keyof ItemFormState['values'], string>>;
}
...
export interface ItemFormProps {
    ...
    onReset?: (formValues: Partial<ItemFormState['values']>) => void;
...
export default function ItemForm(props: ItemFormProps) {
    ...
    const handleTextFieldChange = React.useCallback(
        (event: React.ChangeEvent<HTMLInputElement>) => {
            onFieldChange(
                event.target.name as keyof ItemFormState['values'],
                event.target.value,
            );
        },
        [onFieldChange],
    ...
    const handleCheckboxFieldChange = React.useCallback(
        (event: React.ChangeEvent<HTMLInputElement>, checked: boolean) => {
            onFieldChange(event.target.name as keyof ItemFormState['values'], checked);
        },
        [onFieldChange],
    );

    const handleDateFieldChange = React.useCallback(
        (fieldName: keyof ItemFormState['values']) => (value: Dayjs | null) => {
            if (value?.isValid()) {
