...
export function getItemsStore(): Employee[] {
...
export function setItemsStore(employees: Employee[]) {
...
export async function getMany({
    ...
    const itemsStore = getItemsStore();
    ...
    if (filterModel?.items?.length) {
        filterModel.items.forEach(({ field, value, operator }) => {
            ...
            filteredItems = filteredItems.filter((employee) => {
                const itemValue = employee[field as keyof Employee];
    ...
    const paginatedEmployees = filteredItems.slice(start, end);
...
export async function getOne(employeeId: number) {
    const employeesStore = getItemsStore();
...
export async function createOne(data: Omit<Employee, 'id'>) {
    ...
    setItemsStore([...employeesStore, newEmployee]);
...
export async function deleteOne(employeeId: number) {
    ...
    setItemsStore(employeesStore.filter((employee) => employee.id !== employeeId));
}
